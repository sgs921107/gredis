...
// keys
func (c *Client) ScanIter(pattern string, count int64) (keys []string, err error) {
	cursor := uint64(0)
	for {
...
// 查找集合元素
func (c *Client) SScanIter(key, match string, count int64) (members []string, err error) {
	cursor := uint64(0)
	for {
...
// 查找zset中的元素
func (c *Client) ZScanIter(key, match string, count int64) (members []string, err error) {
	corsur := uint64(0)
	for {
...
// 查找hash中的元素
func (c *Client) HScanIter(key, match string, count int64) (members []string, err error) {
	cursor := uint64(0)
	for {
...
*/
func (c *Client) ZAddRemByRank(key string, length int, members ...Z) *Cmd {
	keys := []string{key}
...
*/
func (c *Client) LPushTrim(key string, length int, values ...interface{}) *Cmd {
	keys := []string{key}
	args := []interface{}{0, length - 1}
...
*/
func (c *Client) RPushTrim(key string, length int, values ...interface{}) *Cmd {
	keys := []string{key}
